...
CGProcedure::emitInfixExpr(InfixExpression *E) {
  ...
  // Приведение типов: если один из операндов i1, а другой i64 → расширяем i1 до i64
  if (Left->getType()->isIntegerTy(1) && Right->getType()->isIntegerTy(64))
      Left = Builder.CreateZExt(Left, Right->getType());
  else if (Left->getType()->isIntegerTy(64) && Right->getType()->isIntegerTy(1))
